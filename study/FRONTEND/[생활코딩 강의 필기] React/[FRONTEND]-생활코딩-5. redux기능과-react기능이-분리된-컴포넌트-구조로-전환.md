...
> 이번 챕터에서는 컨테이너 컴포넌트와 프리젠테이셔널 컴포넌트로 컴포넌트의 계층을 분리한다. 이렇게 하는 이유는 재사용성을 높이기 위해서이다.<br>
>
> 뒤에서 적용하게 될 react-redux는, react에서 사용하는 컴포넌트의 render () 나 props 함수 처리를 하는 보일러플레이트 코드들이 잘 추상화하고 있는 장점이 있다. (마치 스프링의 트랜잭션 추상화, 트랜잭션 템플릿, @Transactional 과 같은 개념이다)<br>
